...
import com.example.paperexchange.repositories.TradeRepository;
...
public class TradeService {
    ...
    public void processPriceUpdate(PriceDto priceDto) {
        String symbol = priceDto.symbol();
        // Process market orders
        Queue<Order> orders = marketBuyOrders.get(symbol);
        while (orders != null && !orders.isEmpty()) {
            executeOrder(orders.remove(), priceDto.price());
        }
        ...
        while (orders != null && !orders.isEmpty() && orders.peek().getExecutionPrice() >= priceDto.price()) {
            executeOrder(orders.remove(), priceDto.price());
        }
